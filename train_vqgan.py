...
from model.vqgan import *
from model.torch_utils import load_data, weights_init
    ...
    def validate(self, val_loader, epoch):
        ...
        with torch.no_grad():
            for imgs, _ in val_loader:
                ...
                val_loss += perceptual_rec_loss.item()

        avg_val_loss = val_loss / len(val_loader)
        print(f"Epoch [{epoch}] - Validation Loss: {avg_val_loss}")

        # 将验证损失写入txt文件
        with open('val_loss.txt', 'a') as f:  # 'a' 代表以追加模式打开文件
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train-batch-size', type=int, default=6, help='Input batch size for training (default: 6)')
